...
// monetary amount.
const (
	AmountMegaFLO  AmountUnit = 6
	AmountKiloFLO  AmountUnit = 3
	AmountFLO      AmountUnit = 0
	AmountMilliFLO AmountUnit = -3
	AmountMicroFLO AmountUnit = -6
	AmountSatoshi  AmountUnit = -8
)

// String returns the unit as a string.  For recognized units, the SI
// prefix is used, or "Satoshi" for the base unit.  For all unrecognized
// units, "1eN FLO" is returned, where N is the AmountUnit.
func (u AmountUnit) String() string {
	...
	case AmountSatoshi:
		return "Satoshi"
	default:
		return "1e" + strconv.FormatInt(int64(u), 10) + " FLO"
	}
}
...
//
// NewAmount is for specifically for converting FLO to Satoshi.
// For creating a new Amount with an int64 value which denotes a quantity of Satoshi,
// do a simple type conversion from type int64 to Amount.
// See GoDoc for example: http://godoc.org/github.com/bitspill/floutil#example-Amount
...
// ToFLO is the equivalent of calling ToUnit with AmountFLO.
func (a Amount) ToFLO() float64 {
	return a.ToUnit(AmountFLO)
}
...
// String is the equivalent of calling Format with AmountFLO.
func (a Amount) String() string {
	return a.Format(AmountFLO)
}
