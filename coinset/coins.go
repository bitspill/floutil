...
func (cs *CoinSet) TotalValue() (value floutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue floutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue floutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount floutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue floutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue floutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue floutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue floutil.Amount, coins []Coin) (Coins, error) {
...
// floutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() floutil.Amount {
	return floutil.Amount(c.txOut().Value)
}
