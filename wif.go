...
func NewWIF(privKey *floec.PrivateKey, net *chaincfg.Params, compress bool) (*WIF, error) {
	if net == nil {
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	switch decodedLen {
	case 1 + floec.PrivKeyBytesLen + 1 + 4:
		if decoded[33] != compressMagic {
			return nil, ErrMalformedPrivateKey
		}
		compress = true
	case 1 + floec.PrivKeyBytesLen + 4:
		compress = false
	default:
		return nil, ErrMalformedPrivateKey
	}
	...
	var tosum []byte
	if compress {
		tosum = decoded[:1+floec.PrivKeyBytesLen+1]
	} else {
		tosum = decoded[:1+floec.PrivKeyBytesLen]
	}
	...
	netID := decoded[0]
	privKeyBytes := decoded[1 : 1+floec.PrivKeyBytesLen]
	privKey, _ := floec.PrivKeyFromBytes(floec.S256(), privKeyBytes)
...
func (w *WIF) SerializePubKey() []byte {
	pk := (*floec.PublicKey)(&w.PrivKey.PublicKey)
	if w.CompressPubKey {
